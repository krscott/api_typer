...
struct Opt {
    ...
    #[structopt(short, long, parse(from_os_str), help = "Output Python File")]
...
fn app(opt: Opt) -> Result<(), Box<dyn Error>> {
    ...
    if let Some(py_path) = &opt.python {
        log::debug!("Generating Python: {:?}", py_path);
        let python_str = format!(
            "# Auto-generated by api_typer\n\n{}\n",
            api_typer::to_python(&spec)
        );
        FileOrStdout::write_all(py_path, python_str.as_bytes())?;
    }
